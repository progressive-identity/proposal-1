...
class Store():
    ...
    def query_order(self, sess, *kargs):
        ...
        return sess.query(*kargs)\
            .filter(
                Order.sign_date <= now,
                sql.or_(
                    Order.expiration == None,       # noqa E711
                    now <= Order.expiration
                ),
                sql.or_(
                    Order.revocation_date == None,  # noqa E711
                    now <= Order.revocation_date
                ))
    ...
    def is_bound(self, user_k, authz_k, rsrc_k):
        try:
            self.query_order_bind(self.session(), Bind.id)\
                .filter(
