...
class File:
    def __init__(self, path, hashname=None, _hash=None):
        self.path = path
...
def dump(root_path, it):
    ...
    # XXX include small collections directly into the root index?
...
def main():
    ...
    r = list(r)
    print(json.dumps(r, sort_keys=True, indent=2, default=json_default))
    print(f"{len(r)} result(s)", file=sys.stderr)
